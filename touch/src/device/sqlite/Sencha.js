 ...
 * @private
 */
Ext.define('Ext.WmsDroid.sqlite.Sencha', {
    /**
     * Returns a {@link Ext.WmsDroid.SQLite.Database} instance. If the database with specified name does not exist, it will be created.
     ...
     *
     * @param {Ext.WmsDroid.SQLite.Database} config.creationCallback.database
     ...
     *
     * @return {Ext.WmsDroid.SQLite.Database}
     * The opened database, null if an error occured.
     */
    openDatabase: function(config) {
        if (config.name == null) {
            Ext.Logger.error('Ext.WmsDroid.SQLite#openDatabase: You must specify a `name` of the database.');
            return null;
        }

        if (config.version == null) {
            Ext.Logger.error('Ext.WmsDroid.SQLite#openDatabase: You must specify a `version` of the database.');
            return null;
        }

        if (config.displayName == null) {
            Ext.Logger.error('Ext.WmsDroid.SQLite#openDatabase: You must specify a `displayName` of the database.');
            return null;
        }

        if (config.estimatedSize == null) {
            Ext.Logger.error('Ext.WmsDroid.SQLite#openDatabase: You must specify a `estimatedSize` of the database.');
            return null;
        }

        var database = null;

        var result = Ext.WmsDroid.Communicator.send({
        ...
        if (result) {
            ...
            database = Ext.create('Ext.WmsDroid.SQLite.Database', result.id, result.version);
     ...
     */
    Ext.define('Ext.WmsDroid.SQLite.Database', {
        id: 0,
        version: null,
         ...
         */
        getVersion: function() {
            return Ext.WmsDroid.Communicator.send({
        ...
        /**
         * Performs a {@link Ext.WmsDroid.SQLite.SQLTransaction} instance with a read/write mode.
         *
         * @param {Object} config
         * The object which contains the following config options:
         *
         ...
         *
         * @param {Ext.WmsDroid.SQLite.SQLTransaction} config.callback.transaction
        ...
        transaction: function(config) {
            if (!config.callback) {
                Ext.Logger.error('Ext.WmsDroid.SQLite.Database#transaction: You must specify a `callback` callback.');
                return null;
            }
            ...
            Ext.WmsDroid.Communicator.send({
                ...
                callbacks: {
                    success: function(id) {
                        var exception = null;
                        var error = null;
                        var transaction = Ext.create('Ext.WmsDroid.SQLite.SQLTransaction', id);

                        error = Ext.WmsDroid.Communicator.send({
                        ...
                        while (!(exception || error) && statements.length > 0) {
                            ...
                            if (result) {
                                if (result.error) {
                                    ...
                                    var resultSet = Ext.create('Ext.WmsDroid.SQLite.SQLResultSet', result);
        ...
        /**
         * Works same as {@link Ext.WmsDroid.SQLite.Database#transaction}, but performs a {@link Ext.WmsDroid.SQLite.SQLTransaction} instance with a read-only mode.
         */
        ...
        /**
         * Verifies and changes the version of the database at the same time as doing a schema update with a {@link Ext.WmsDroid.SQLite.SQLTransaction} instance.
         *
         * @param {Object} config
         * The object which contains the following config options:
         *
        ...
        changeVersion: function(config) {
            if (config.oldVersion == null) {
                Ext.Logger.error('Ext.WmsDroid.SQLite#changeVersion: You must specify an `oldVersion` of the database.');
                return null;
            }

            if (config.newVersion == null) {
                Ext.Logger.error('Ext.WmsDroid.SQLite#changeVersion: You must specify a `newVersion` of the database.');
                return null;
            }
         ...
         */
        Ext.define('Ext.WmsDroid.SQLite.SQLTransaction', {
            id: 0,
            active: false,
            statements: null,
             ...
             *
             * @param {Ext.WmsDroid.SQLite.SQLTransaction} config.callback.resultSet
             ...
             *
             * @param {Ext.WmsDroid.SQLite.SQLTransaction} config.failure.transaction
             * The transaction of the SQL statement.
             *
             * @param {Object} config.failure.error
             * The occurred error.
            ...
            executeSql: function(config) {
                if (!this.active) {
                    Ext.Logger.error('Ext.WmsDroid.SQLite.SQLTransaction#executeSql: An attempt was made to use a SQLTransaction that is no longer usable.');
                    return null;
                }

                if (config.sqlStatement == null) {
                    Ext.Logger.error('Ext.WmsDroid.SQLite.SQLTransaction#executeSql: You must specify a `sqlStatement` for the transaction.');
                    return null;
                }
             ...
             */
            Ext.define('Ext.WmsDroid.SQLite.SQLResultSet', {
                insertId: 0,
                rowsAffected: 0,
                rows: null,
                ...
                constructor: function(data) {
                    ...
                    this.rows = Ext.create('Ext.WmsDroid.SQLite.SQLResultSetRowList', data);
                },
                ...
                getInsertId: function() {
                    if (this.insertId != 0) {
                        ...
                        Ext.Logger.error('Ext.WmsDroid.SQLite.SQLResultSet#getInsertId: An SQLTransaction did not insert a row.');
                        return null;
                    }
                },
                ...
                /**
                 * Returns a {@link Ext.WmsDroid.SQLite.SQLResultSetRowList} instance representing rows returned by the SQL statement.
                 *
                 * @return {Ext.WmsDroid.SQLite.SQLResultSetRowList}
                 * The rows.
                 */
                getRows: function() {
                    return this.rows;
                 ...
                 */
                Ext.define('Ext.WmsDroid.SQLite.SQLResultSetRowList', {
                    names: null,
                    rows: null,
