 ...
 *
 * As an example, to capture a photo from the WmsDroid's camera, we use `Ext.WmsDroid.Camera.capture()` like:
 *
 ...
 *     );
 *
 * Internally, `Ext.WmsDroid.Communicator.send()` will then be invoked with the following argument:
 *
 ...
 * respectively) and maintained by `Ext.WmsDroid.Communicator`.
 *
 * Whenever the requested operation finishes, `Ext.WmsDroid.Communicator.invoke()` simply needs
 * to be executed from the native shell with the corresponding ids given before. For example:
 *
 *     Ext.WmsDroid.Communicator.invoke('3', ['DATA_URI_OF_THE_CAPTURED_IMAGE_HERE']);
 *
 ...
 *
 * Note that `Ext.WmsDroid.Communicator` maintains the uniqueness of each function callback and
 * its scope object. If subsequent calls to `Ext.WmsDroid.Communicator.send()` have the same
 ...
 */
Ext.define('Ext.WmsDroid.communicator.Default', {
